...
def loglike(x):
    ...
    logl_obsframe = grbpop.Ppop.biased_obsframe_loglikelihood(p50300,epbias=efix,alpha=alpha,specmodel='Comp',inst='Fermi',theta_pop=theta_pop,res=80,pdet=pdet,pflim=p_gbm_lim,return_logalpha=False)
    
    ## restframe sample
    logl_restframe = grbpop.Ppop.biased_restframe_loglikelihood(Lsamples[:-1],zobs[:-1],epbias=efix,alpha=alpha,inst='Fermi+Swift',theta_pop=theta_pop,specmodel='Comp',pdet=None,pflim=[p_gbm_lim,p_swift_lim],prior_EpLz=pi_EpLz,logalpha=None,res=60)
...
if __name__=='__main__':
    nthreads = 8
    N_iter = 10000
    chain_filename = 'chains/SGRB_flux-limited-sample-analysis_WRONGCUT_dtdsfh_log_fixed_ep.h5' # full
    
    # initial guess vector for 'dtd':'lognorm'
    #      log(thj)  log(Lj) a_L      b_L   log(Epj) a_Ep    b_Ep  log(thw)  A       log(s_c)    y    mu_td    sigma_td   
    x0 = [-1.877,     51.55, 4.091, -2.318, 3.804,    1.2,   2.069, -0.5058, 3.041, -0.01476, -0.1149,    2.,       1.]  
