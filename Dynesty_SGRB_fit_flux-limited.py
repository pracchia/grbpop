...
def ptform(u):
    ...
    # x[0] = u[0]*np.log10(np.pi/2./0.01) + np.log10(0.01) ### !!!
    ...
    # x[7] = u[7]*(np.log10(np.pi/2.)-x[0]) + x[0] ###
...
if __name__=='__main__':
    ...
    with dypool.Pool(nthreads, loglike=loglike, prior_transform=ptform) as pool:
        if os.path.exists(checkpoint_filename):
            ...
            dsampler.run_nested(resume=True, checkpoint_file=checkpoint_filename)
