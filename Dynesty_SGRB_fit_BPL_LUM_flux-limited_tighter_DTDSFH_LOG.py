...
def ptform(u):
    ...
    x[3] = u[3]*(logLmax-51.) + 51. # scale and shift to [log10(1e51), logLmax]
    ...
    # LOG PRIOR VERSION
    # # 'mu_td':x[9], theta_pop['mu_td']<0.01, theta_pop['mu_td']>5.
    x[9] = u[9]*np.log10(5/0.01) + np.log10(0.01)
    # x[9] = u[9]*(5.-0.01) + 0.01 # scale and shift to [0.01, 5]

    # LOG PRIOR VERSION
    # # 'sigma_td':x[10], theta_pop['sigma_td']<0.01, theta_pop['sigma_td']>5.
    x[10] = u[10]*np.log10(5/0.01) + np.log10(0.01)
    # x[10] = u[10]*(5.-0.01) + 0.01 # scale and shift to [0.01, 5]
    
    # 'R0':10**x[11], theta_pop['R0']<1., theta_pop['R0']>1e4:
    x[11] = u[11]*4. # scale and shift to [log10(1.), log10(1e4)]
...
def loglike(x):
    ...
    logl_obsframe, log_alpha_obsframe = grbpop.Ppop.obsframe_loglikelihood_lum_2breaks(p50300,ep,alpha=alpha,specmodel='Comp',inst='Fermi',theta_pop=theta_pop,res=100,pdet=pdet,pflim=p_gbm_lim,return_logalpha=True)

    ## restframe sample
    logl_restframe = grbpop.Ppop.restframe_loglikelihood_lum2breaks(Lsamples[:-1],Epsamples[:-1],zobs[:-1],alpha=alpha,inst='Fermi+Swift',theta_pop=theta_pop,specmodel='Comp',pdet=None,pflim=[p_gbm_lim,p_swift_lim],prior_EpLz=pi_EpLz,logalpha=None,res=100)

## joint detection sample
    # logl_joint_detection, log_alpha_joint_detection = grbpop.Ppop.joint_detection_likelihood_lum2breaks(Lsamples[-1],Epsamples[-1],z_NGC4993,alpha=alpha,specmodel='Comp',inst='Fermi',theta_pop=theta_pop,res=100,pdet=pdet,pflim=None,prior_EpLz=pi_EpLz,return_logalpha=True)
    ...
    # log_poisson_joint_detection = grbpop.Ppop.log_poissonian_joint_detection(theta_pop,N_obs=1,eta=eta_GBM,T=T_LVK,logalpha=log_alpha_joint_detection)
    
    ## sum all contributions
    logl = logl_obsframe + logl_restframe + log_poisson_obsframe
    # logl = logl_obsframe + logl_restframe + logl_joint_detection + log_poisson_obsframe + log_poisson_joint_detection
...
if __name__=='__main__':
    ...
    # checkpoint_filename = f'nested_samplings/draft_nested_BPL_flux-limited_tighter_Poisson_dtdsfh_LOG_dlogz{dlogz}_nlive{nlive}_nbatch{nbatch}_neff{N_effective_sample}.save' # Linear DTD params
    checkpoint_filename = f'nested_samplings/final_nested_BPL_flux-limited_tighter_Poisson_dtdsfh_LOG_dlogz{dlogz}_nlive{nlive}_nbatch{nbatch}_neff{N_effective_sample}.save' # Log DTD params
