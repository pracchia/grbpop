...
#########################################################
### Diagnose for Broken power law luminosity function ###
#########################################################

def BPL_L_Ep_z_contours_and_cumulatives(theta_pop=Ppop.default_theta_pop,pflim=5.,inst='Fermi',alpha=-0.5,specmodel='Comp',res=50,smooth=0.5,pdet='gbm',pdetGW=None):
    L = np.logspace(logLmin,logLmax,res+1)
    Ep = np.logspace(logEpmin,logEpmax,res)
    z = np.logspace(logzmin,logzmax,res-1)
...
def BPL_pf_ep_contours_and_cumulatives(theta_pop=Ppop.default_theta_pop,pflim=0.01,inst='Fermi',alpha=-0.5,specmodel='Comp',res=50,smooth=0.5,pdet='gbm'):
    
    L0 = np.logspace(logLmin,logLmax,res+1)
    Ep0 = np.logspace(logEpmin,logEpmax,res)
    
    PEpL = Ppop.PEpL_iso(L0,Ep0,theta_pop)
    # PEpL = Ppop.PEpL(L0,Ep0,theta_pop)
    Itp_logPEpL = RegularGridInterpolator(points=(np.log10(Ep0),np.log10(L0)),values=np.log10(PEpL),bounds_error=False,fill_value=-np.inf)
